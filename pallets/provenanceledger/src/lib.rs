...
//! ## Overview
//!
//! This pallet emits provenance activity events for single-subject and grouped-subject actions.
//! It is intended to be integrated into a runtime and upgraded onto a running network using
//! `sudo -> system.setCode`.
//!
//! ## Interface
//!
...
//!
//! * `add_activity` - Emit provenance activity events for a list of subject IDs.
//! * `add_activity_group` - Emit provenance activity events for grouped subject IDs.
...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		#[pallet::call_index(1)]
		#[pallet::weight({1})]
		pub fn add_activity_group(
			origin: OriginFor<T>,
			action_id: Vec<u8>,
			number_of_subject_ids: u8,
		) -> DispatchResult {
			let sender = ensure_signed(origin)?;
